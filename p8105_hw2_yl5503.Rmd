...
str(final_df)
```
The `pols-month.csv` dataset contains $`r ncol(pols_df)`$ variables and $`r nrow(pols_df)`$ observations. The `pols-month.csv` dataset tells us about the party affiliation distribution (democrat or republican) for governors and senators for a given year from years $`r range(pols_df$year)[1]`$ to $`r range(pols_df$year)[2]`$. It also tells us whether the sitting president was a democrat or republican.

The `snp.csv` dataset contains $`r ncol(snp_df)`$ variables and $`r nrow(snp_df)`$ observations ranging from years $`r range(snp_df$year)[1]`$ to $`r range(snp_df$year)[2]`$. The `snp.csv` dataset tells us the closing value of the S&P stock index on the associated date. The three variables are `month`, `day` and `close`. The average `close` value is $$`r mean(snp_df$close)`$. 
